...
class ContainerRestarted extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("OpenRail: A resource ({$this->name}) has been restarted automatically on {$this->server->name}");
    ...
    {
        $message = "OpenRail: A resource ({$this->name}) has been restarted automatically on {$this->server->name}";
        $payload = [
            'message' => $message,
        ];
        if ($this->url) {
